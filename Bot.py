...
def Main():
    ...
    if ((login == 'SupaBot') and (password == 'RearmHexBlink')):
        threadCMD = threading.Thread(target=commandLine)
        threadBOT = threading.Thread(target=MSG)
        threadCMD.start()
        threadBOT.start()
    elif ((login == 'AlexReead') and (password == 'stray228')):
        threadCMD = threading.Thread(target=commandLine)
        threadMSG = threading.Thread(target=AdditionalMSG)
        threadMSG.start()
        threadCMD.start()
    elif((login == 'SupaEnot') and (password == 'a1ta0da')):
        threadCMD = threading.Thread(target=commandLine)
        threadMSG = threading.Thread(target=AdditionalMSG)
        threadMSG.start()
        threadCMD.start()
    else:
        print('-->WARNING: NOT SUSEFUL LOGINING:WARNING<--')
